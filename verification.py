...
class verifier(object):
    ...
    def verifyOther(self, tx):
        try:
            time.sleep(60)
            verified = self.pwW.tx(tx['id'])

            if verified['height'] > 0:
                values = ("WAVES", tx['id'], verified['height'])
                cursor = self.dbCon.cursor()
                cursor.execute('INSERT INTO verified ("chain", "tx", "block") VALUES (?, ?, ?)', values)
                self.dbCon.commit()
                print('tx to waves verified!')
            else:
                values = ("WAVES", tx['id'], 0)
                cursor = self.dbCon.cursor()
                cursor.execute('INSERT INTO verified ("chain", "tx", "block") VALUES (?, ?, ?)', values)
                self.dbCon.commit()
                print('tx to waves not verified!')
        except:
            values = ("WAVES", tx['id'], 0)
            cursor = self.dbCon.cursor()
            cursor.execute('INSERT INTO verified ("chain", "tx", "block") VALUES (?, ?, ?)', values)
